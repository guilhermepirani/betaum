...
def index():
    ...
    stocks = db.execute("SELECT stock, shares FROM portfolio WHERE user_id = :user_id", user_id=user_id)
    ...
    for stock in stocks:
        ...
        list_of_stocks.append({
            'symbol': quote_stock["symbol"],
            'quote': lookup(symbol),
            'name': quote_stock["name"],
            'shares': stock["shares"],
            'price': usd(quote_stock["price"]),
            'total': usd(stock["shares"] * quote_stock["price"])
        })

        # Adding the stocks values to user's holdings
        holdings += stock["shares"] * quote_stock["price"]

    # Formatting to USD
    holdings = usd(holdings)
    cash = usd(cash)

    return render_template("index.html", stocks=list_of_stocks, holdings=holdings, cash=cash)
...
def buy():
    ...
    if request.method == "POST":
        ...
        # Check Stock
        quote_stock = lookup(request.form.get("symbol"))
        if quote_stock is None or quote_stock == "":
        ...
        user_id = session["user_id"]
        price = quote_stock["price"]
        final_price = int(price) * number_of_shares
        available_cash = db.execute("SELECT cash FROM users WHERE id = :user_id", user_id=user_id)[0]["cash"]
        if final_price > available_cash:
            ...
            if db.execute("SELECT * FROM portfolio WHERE user_id = :user_id AND stock = :symbol", user_id=user_id,
                          symbol=quote_stock["symbol"]):
                db.execute(
                    "UPDATE portfolio SET shares = shares + :shares WHERE stock = :symbol AND user_id = :user_id",
                    user_id=user_id, symbol=quote_stock["symbol"], shares=number_of_shares)
            else:
                db.execute("INSERT INTO portfolio (stock, shares, user_id) VALUES (:stock, :shares, :user_id)",
                           stock=quote_stock["symbol"], shares=number_of_shares, user_id=user_id)

            # Update user's cash
            total_price = quote_stock["price"] * number_of_shares
            ...
            date = datetime.now()
            db.execute(
                "INSERT INTO history (stock, shares, price, transacted, user_id) VALUES (:stock, :shares, :price, "
                " :transacted, :user_id)",
                stock=quote_stock["symbol"], shares=number_of_shares, price=total_price, transacted=date,
                user_id=user_id)
...
def quote():
    ...
    if request.method == "POST":
        ...
        if quote_stock is None:
            ...
            price = usd(quote_stock["price"])
            return render_template("quoted.html", quote=quote_stock, price=price)
...
def sell():
    ...
    if request.method == "POST":
        ...
        if stock is None or req_shares is None:
        ...
        # Actually sell
            ...
            quote_stock = lookup(stock)
            date = datetime.now()

            # Update user's cash
            total_price = quote_stock["price"] * int(req_shares)
            db.execute("UPDATE users SET cash = cash + :total_price WHERE id = :user_id",
                       total_price=total_price, user_id=user_id)

            # Update user's portfolio
            db.execute(
                "UPDATE portfolio SET shares = shares - :req_shares WHERE user_id = :user_id AND stock = :symbol",
